...
public class JDBCLibroDigitalDAO extends JDBCGenericDAO<LibroDigital, String> implements LibroDigitalDAO {
	...
	@Override
	public void create(LibroDigital digital) {

		Libro libro = digital;
		libro.calcularPrecio();

		conexionUno.update("INSERT Libros VALUES ('" + digital.getIsbn() + "', '" + digital.getTitulo() + "', '"
				+ digital.getAutor() + "', '" + digital.getEdicion() + "', " + digital.getPrecio() + ", '"
				+ digital.getImagen() + "')");

		conexionUno.update("INSERT libros_digitales (lib_dig_comision, lib_isbn) VALUES (" + digital.getComision()
				+ ", '" + digital.getIsbn() + "')");
